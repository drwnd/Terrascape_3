...
public final class InsideSlopedStairPlaceable extends RotatableShapePlaceable {
    ...
    private boolean isInside(int threshold, int a, int b, int c) {
        return a / stepHeight.value() * slope.value() + Math.min(b, c) / stepHeight.value() < threshold;
    }
