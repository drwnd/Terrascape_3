...
public final class OpenCL {
    ...
    private static void demo(MemoryStack stack) {
        ...
        for (int p = 0; p < platforms.capacity(); p++) {
            ...
            for (int d = 0; d < devices.capacity(); d++) {
                ...
                long context = clCreateContext(ctxProps, device, contextCB = CLContextCallback.create((errinfo, _, _, _) -> {
                ...
                if (caps.OpenCL11) {
                    ...
                    errcode = clSetMemObjectDestructorCallback(subbuffer, subbufferCB = CLMemObjectDestructorCallback.create((memobj, _) -> {
                ...
                if ((exec_caps & CL_EXEC_NATIVE_KERNEL) == CL_EXEC_NATIVE_KERNEL) {
                    ...
                    errcode = clSetEventCallback(e, CL_COMPLETE, eventCB = CLEventCallback.create((event, event_command_exec_status, _) -> {
    ...
    public static void checkCLError(int error) {
        if (error == CL_SUCCESS) return;
        System.out.printf("Error %d at line %s%n", error, new Exception().getStackTrace()[1].toString());
    }
}
